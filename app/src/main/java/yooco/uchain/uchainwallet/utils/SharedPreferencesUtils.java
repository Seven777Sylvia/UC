...
public class SharedPreferencesUtils {
    ...
    private static final String SP_NAME = "sp_uchain";


    public static void putParam(Context context, String key, Object value) {
        if (null == context) {
            UChainLog.e(TAG, "setParam() -> context is null!");
            return;
        }

        if (TextUtils.isEmpty(key) || null == value) {
            UChainLog.e(TAG, "setParam() -> key or value is null!");
            return;
        }

        String clsName = value.getClass().getSimpleName();
        if (TextUtils.isEmpty(clsName)) {
            UChainLog.e(TAG, "setParam() -> key's clsName is null!");
            return;
        }
    ...
    public static Object getParam(Context context, String key, Object defVal) {
        if (null == context) {
            UChainLog.e(TAG, "getParam() -> context is null!");
            return null;
        }

        if (TextUtils.isEmpty(key) || null == defVal) {
            UChainLog.e(TAG, "getParam() -> key or defVal is null!");
            return null;
        }

        String clsName = defVal.getClass().getSimpleName();
        if (TextUtils.isEmpty(clsName)) {
            UChainLog.e(TAG, "getParam() -> key's clsName is null!");
            return null;
        }
    ...
    public static void remove(Context context, String key) {
        if (null == context || TextUtils.isEmpty(key)) {
            UChainLog.e(TAG, "remove() -> context or key is null!");
            return;
        }

        SharedPreferences sp = context.getSharedPreferences(SP_NAME, Context.MODE_PRIVATE);
        if (null == sp) {
            UChainLog.e(TAG, "sp is null!");
            return;
        }
