...
import yooco.uchain.uchainwallet.data.bean.PortraitShow;
...
public class UChainWalletDbDao {

    private static final String TAG = UChainWalletDbDao.class.getSimpleName();

//    private static final String DB_PWD = "";

    private static UChainWalletDbDao sUChainWalletDbDao;

    private AtomicInteger mOpenCounter = new AtomicInteger();

    private UChainWalletDbHelper mApexWalletDbHelper;

    private SQLiteDatabase mDatabase;

    private UChainWalletDbDao(Context context) {
        mApexWalletDbHelper = new UChainWalletDbHelper(context);
    }

    public static UChainWalletDbDao getInstance(Context context) {
        if (null == context) {
            UChainLog.e(TAG, "context is null!");
            return null;
        }

        if (null == sUChainWalletDbDao) {
            synchronized (UChainWalletDbDao.class) {
                if (null == sUChainWalletDbDao) {
                    sUChainWalletDbDao = new UChainWalletDbDao(context);
                }
            }
        }

        return sUChainWalletDbDao;
    }
    ...
    public synchronized void insert(String tableName, WalletBean walletBean) {
        if (TextUtils.isEmpty(tableName) || null == walletBean) {
            UChainLog.e(TAG, "insert() -> tableName or walletBean is null!");
            return;
        }
        ...
        try {
            ...
            UChainLog.i(TAG, "insert() -> insert " + walletBean.getName() + " ok!");
        } catch (SQLException e) {
            UChainLog.e(TAG, "insert exception:" + e.getMessage());
        } finally {
            db.endTransaction();
        }
        closeDatabase();
    ...
    public void deleteByWalletNameAndAddr(String tableName, String walletName, String walletAddress) {
        if (TextUtils.isEmpty(walletName) || TextUtils.isEmpty(walletAddress)) {
            UChainLog.e(TAG, "deleteByWalletNameAndAddr() -> walletName or walletAddress is null!");
            return;
        }
        ...
        try {
            ...
            db.setTransactionSuccessful();
            UChainLog.i(TAG, "deleteByWalletNameAndAddr() -> delete " + walletName + ":" + walletAddress + " ok!");
        } catch (Exception e) {
            UChainLog.e(TAG, "deleteByWalletNameAndAddr exception:" + e.getMessage());
        } finally {
            db.endTransaction();
        }
        closeDatabase();
    ...
    public List<WalletBean> queryWallets(String tableName) {
        if (TextUtils.isEmpty(tableName)) {
            UChainLog.e(TAG, "tableName is null!");
            return null;
        }
    ...
    public WalletBean queryByWalletAddress(String tableName, String walletAddress) {
        if (TextUtils.isEmpty(tableName)
            ...
            UChainLog.e(TAG, "queryByWalletName() -> tableName or walletAddress is null!");
            return null;
        }
    ...
    public void updateBackupState(String tableName, String walletAddress, int backupState) {
        if (TextUtils.isEmpty(tableName) || TextUtils.isEmpty(walletAddress)) {
            UChainLog.e(TAG, "updateBackupState() -> tableName or walletAddress is null!");
            return;
        }
        ...
        try {
            ...
            db.setTransactionSuccessful();
            UChainLog.i(TAG, "updateBackupState -> update" + walletAddress + " backupState ok!");
        } catch (SQLException e) {
            UChainLog.e(TAG, "updateBackupState exception:" + e.getMessage());
        } finally {
            db.endTransaction();
        }
        closeDatabase();
    ...
    public void updateWalletName(String tableName, String walletAddress, String walletNameNew) {
        if (TextUtils.isEmpty(tableName)
            ...
            UChainLog.e(TAG, "tableName or walletAddress or walletNameNew is null!");
            return;
        }
        ...
        try {
            ...
            db.update(tableName, contentValues, WHERE_CLAUSE_WALLET_ADDRESS_EQ, new String[]{walletAddress});
            db.setTransactionSuccessful();
            UChainLog.i(TAG, "updateWalletName: " + walletNameNew + " is ok!");
        } catch (SQLException e) {
            UChainLog.e(TAG, "updateBackupStateByWalletName exception:" + e.getMessage());
        } finally {
            db.endTransaction();
        }
        closeDatabase();
    ...
    public void updateCheckedAssets(String tableName, WalletBean walletBean) {
        if (TextUtils.isEmpty(tableName) || null == walletBean) {
            UChainLog.e(TAG, "tableName or walletBean is null!");
            return;
        }

        String walletAddress = walletBean.getAddress();
        if (TextUtils.isEmpty(walletAddress)) {
            UChainLog.e(TAG, "walletAddress is null!");
            return;
        }
        ...
        try {
            ...
            db.update(tableName, contentValues, WHERE_CLAUSE_WALLET_ADDRESS_EQ, new String[]{walletAddress});
            db.setTransactionSuccessful();
            UChainLog.i(TAG, "updateCheckedAssets is ok!");
        } catch (SQLException e) {
            UChainLog.e(TAG, "updateCheckedAssets exception:" + e.getMessage());
        } finally {
            db.endTransaction();
        }
        closeDatabase();
    ...
    public synchronized void insertTxRecord(String tableName, TransactionRecord transactionRecord) {
        if (TextUtils.isEmpty(tableName) || null == transactionRecord) {
            UChainLog.e(TAG, "insertTxRecord() -> tableName or transactionRecord is null!");
            return;
        }
        ...
        try {
            ...
            db.insertOrThrow(tableName, null, contentValues);
            db.setTransactionSuccessful();
            UChainLog.i(TAG, "insertTxRecord() -> insert " + transactionRecord.getTxID() + " ok!");
        } catch (SQLException e) {
            UChainLog.e(TAG, "insertTxRecord exception:" + e.getMessage());
        } finally {
            db.endTransaction();
        }
        closeDatabase();
    ...
    public List<TransactionRecord> queryTxByAddress(String tableName, String walletAddress) {
        ...
        if (TextUtils.isEmpty(tableName) || TextUtils.isEmpty(walletAddress)) {
            UChainLog.e(TAG, "queryTxByAddress() -> tableName or walletAddress is null!");
    ...
    public HashMap<String, TransactionRecord> queryTxCacheByAddress(String tableName, String walletAddress) {
        ...
        if (TextUtils.isEmpty(tableName) || TextUtils.isEmpty(walletAddress)) {
            UChainLog.e(TAG, "queryTxCacheByAddress() -> tableName or walletAddress is null!");
            return txHashMap;
        }
    ...
    public List<TransactionRecord> queryTxCacheByTxId(String tableName, String txId) {
        ...
        if (TextUtils.isEmpty(tableName) || TextUtils.isEmpty(txId)) {
            UChainLog.e(TAG, "queryTxCacheByTxId() -> tableName or txId is null!");
            return transactionRecords;
        }
    ...
    public synchronized List<TransactionRecord> queryTxByTxIdAndAddress(String tableName, String
        ...
        if (TextUtils.isEmpty(tableName)
                ...
                || TextUtils.isEmpty(walletAddress)) {
            UChainLog.e(TAG, "queryTxByTxIdAndAddress() -> tableName or txId or walletAddress is " +
                    "null!");
            return transactionRecords;
        }
    ...
    public void delCacheByTxIDAndAddr(String tableName, String txID, String walletAddress) {
        if (TextUtils.isEmpty(tableName)
                || TextUtils.isEmpty(txID)
                || TextUtils.isEmpty(walletAddress)) {
            UChainLog.e(TAG, "delCacheByTxIDAndAddr() -> tableName or txID or walletAddress is null!");
            return;
        }
        ...
        try {
            ...
            db.delete(tableName,
                    ...
                    new String[]{txID, walletAddress});
            db.setTransactionSuccessful();
            UChainLog.i(TAG, "delCacheByTxIDAndAddr() -> delete:" + txID + " ok!");
        } catch (Exception e) {
            UChainLog.e(TAG, "delCacheByTxIDAndAddr exception:" + e.getMessage());
        } finally {
            db.endTransaction();
        }
        closeDatabase();
    ...
    public void delTxsByAddress(String tableName, String walletAddress) {
        if (TextUtils.isEmpty(tableName) || TextUtils.isEmpty(walletAddress)) {
            UChainLog.e(TAG, "delTxsByAddress() -> tableName or walletAddress is null!");
            return;
        }
        ...
        try {
            ...
            db.setTransactionSuccessful();
            UChainLog.i(TAG, "delTxsByAddress() -> delete txs of the " + walletAddress + " from " +
                    tableName + " ok!");
        } catch (Exception e) {
            UChainLog.e(TAG, "delTxsByAddress exception:" + e.getMessage());
        } finally {
            db.endTransaction();
        }
        closeDatabase();
    ...
    public void delCacheByTxId(String tableName, String txId) {
        if (TextUtils.isEmpty(tableName) || TextUtils.isEmpty(txId)) {
            UChainLog.e(TAG, "delCacheByTxId() -> tableName or txId is null!");
            return;
        }
        ...
        try {
            ...
            db.setTransactionSuccessful();
            UChainLog.i(TAG, "delCacheByTxId() -> delete:" + txId + " ok!");
        } catch (Exception e) {
            UChainLog.e(TAG, "delCacheByTxId exception:" + e.getMessage());
        } finally {
            db.endTransaction();
        }
        closeDatabase();
    ...
    public List<TransactionRecord> queryTxByState(String tableName, int txState) {
        ...
        if (TextUtils.isEmpty(tableName)) {
            UChainLog.e(TAG, "queryTxByState() -> tableName is null!");
            return transactionRecords;
        }
    ...
    public void updateTxState(String tableName, String txID, int txState) {
        if (TextUtils.isEmpty(tableName) || TextUtils.isEmpty(txID)) {
            UChainLog.e(TAG, "getRawTransaction() -> tableName or txID is null!");
            return;
        }
        ...
        try {
            ...
            db.update(tableName, contentValues, WHERE_CLAUSE_TX_ID_EQ,
                    new String[]{txID});
            db.setTransactionSuccessful();
            UChainLog.i(TAG, "getRawTransaction -> update: " + txID + " ok!");
        } catch (SQLException e) {
            UChainLog.e(TAG, "getRawTransaction exception:" + e.getMessage());
        } finally {
            db.endTransaction();
        }
        closeDatabase();
    ...
    public synchronized void insertAsset(String tableName, AssetBean assetBean) {
        if (TextUtils.isEmpty(tableName) || null == assetBean) {
            UChainLog.e(TAG, "insertAsset() -> tableName or assetBean is null!");
            return;
        }
        ...
        try {
            ...
            db.insertOrThrow(tableName, null, contentValues);
            db.setTransactionSuccessful();
            UChainLog.i(TAG, "insert() -> insert " + assetBean.getSymbol() + " ok!");
        } catch (SQLException e) {
            UChainLog.e(TAG, "insert exception:" + e.getMessage());
        } finally {
            db.endTransaction();
    ...
    public List<AssetBean> queryAssetsByType(String tableName, String assetType) {
        if (TextUtils.isEmpty(tableName) || TextUtils.isEmpty(assetType)) {
            UChainLog.e(TAG, "tableName or assetType is null!");
            return null;
        }
    ...
    public AssetBean queryAssetByHash(String tableName, String assetHexHash) {
        if (TextUtils.isEmpty(tableName) || TextUtils.isEmpty(assetHexHash)) {
            UChainLog.e(TAG, "queryAssetByHash() -> tableName or assetHexHash is null!");
            return null;
        }
    ...
    public synchronized void insertPortraitShow(PortraitShow portraitShow) {
        if (null == portraitShow) {
            UChainLog.e(TAG, "insertPortraitShow() -> portraitShow is null!");
            return;
        }
        ...
        try {
            ...
            UChainLog.e(TAG, "insertPortraitShow exception:" + e.getMessage());
        } finally {
            db.endTransaction();
        }
        closeDatabase();
    ...
    public HashMap<String, PortraitShow> queryPortraitShowByType(String portraitType) {
        ...
        if (TextUtils.isEmpty(portraitType)) {
            UChainLog.e(TAG, "queryPortraitShowByType() -> portraitType is null!");
            return hm;
        }
