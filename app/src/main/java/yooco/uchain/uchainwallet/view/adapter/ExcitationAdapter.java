...
import static yooco.uchain.uchainwallet.R.drawable.new_event_status;
import static yooco.uchain.uchainwallet.R.drawable.new_event_status_end;
...
public class ExcitationAdapter extends RecyclerView.Adapter<ExcitationAdapter.ExcitationAdapterHolder> implements View
    ...
    public void onBindViewHolder(@NonNull ExcitationAdapterHolder holder, int position) {
        ...
        if (TextUtils.isEmpty(bean.getNewEventPic())) {
            ...
            Glide.with(UChainWalletApplication.getInstance()).load(bean.getNewEventPic()).into(holder.newEventPic);
        }
        ...
        if (newEventStatus == Constant.EXCITATION_EXCITATION_AOUBT_TO_BEGIN) {
            holder.newEventStatus.setText(UChainWalletApplication.getInstance().getResources().getText(R.string
                    .excitation_about_to_begin));
            holder.newEventStatus.setBackground(UChainWalletApplication.getInstance().getResources().getDrawable
                    (new_event_status_end));
            holder.newEventStatus.setTextColor(UChainWalletApplication.getInstance().getResources().getColor(R.color.c_ff666666));
        } else if (newEventStatus == Constant.EXCITATION_EXCITATION_IN_PROGRESS) {
            holder.newEventStatus.setText(UChainWalletApplication.getInstance().getResources().getText(R.string
                    .excitation_in_progress));
            holder.newEventStatus.setBackground(UChainWalletApplication.getInstance().getResources().getDrawable(new_event_status));
            holder.newEventStatus.setTextColor(UChainWalletApplication.getInstance().getResources().getColor(R.color.c_FFFFFF));
        } else {
            holder.newEventStatus.setText(UChainWalletApplication.getInstance().getResources().getText(R.string.excitation_closed));
            holder.newEventStatus.setBackground(UChainWalletApplication.getInstance().getResources().getDrawable
                    (new_event_status_end));
            holder.newEventStatus.setTextColor(UChainWalletApplication.getInstance().getResources().getColor(R.color.c_ff666666));
        }
